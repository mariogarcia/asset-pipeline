...
abstract class AbstractUrlRewritingProcessor extends AbstractProcessor {
    ...
    protected String replacementUrl(final AssetFile assetFile, final String url) {
        ...
        final String assetPath  = normalizePath(assetFile.parentPath ? assetFile.parentPath + AssetHelper.DIRECTIVE_FILE_SEPARATOR + urlSplitter.path : urlSplitter.path)
        ...
        final List<String> baseRelativePath = baseFileParentPath ? baseFileParentPath.split(AssetHelper.DIRECTIVE_FILE_SEPARATOR).findAll {it}.reverse() : []
        final List<String> currRelativePath = currFileParentPath ? currFileParentPath.split(AssetHelper.DIRECTIVE_FILE_SEPARATOR).findAll {it}.reverse() : []
        ...
        for (; basePathIndex >= 0; basePathIndex--) {
            replacementPathSb << '..' << AssetHelper.DIRECTIVE_FILE_SEPARATOR
        }

        for (; currPathIndex >= 0; currPathIndex--) {
            replacementPathSb << currRelativePath[currPathIndex] << AssetHelper.DIRECTIVE_FILE_SEPARATOR
        }
