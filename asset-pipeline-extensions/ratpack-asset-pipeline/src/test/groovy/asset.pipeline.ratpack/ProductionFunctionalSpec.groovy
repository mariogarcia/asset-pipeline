...
class ProductionFunctionalSpec extends Specification {
  ...
  EmbeddedApp app = of({ spec ->
    spec
        .serverConfig(ServerConfig.embedded().baseDir(TestConstants.PROD_BASE_DIR.toAbsolutePath()).development(false))
  ...
  void "should serve prod files"() {
    ...
    response.body.text == TestConstants.PROD_BASE_DIR.resolve("assets/index.html").text
  }
  ...
  void "gzipped accept-encoding should be respected"() {
    ...
    def assetBytes = TestConstants.PROD_BASE_DIR.resolve("assets/index.html.gz").bytes
