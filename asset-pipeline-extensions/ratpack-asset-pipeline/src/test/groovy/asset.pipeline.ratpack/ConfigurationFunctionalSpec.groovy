...
class ConfigurationFunctionalSpec extends Specification {
  ...
  EmbeddedApp app = of({ spec -> spec
      .serverConfig(ServerConfig.embedded().baseDir(TestConstants.BASE_DIR.toAbsolutePath()))
  ...
  void "should serve content from the configured url"() {
    ...
    response.body.text.trim() == TestConstants.BASE_DIR.resolve("../assets/html/index.html").text
  }
  ...
  void "should serve index file from configured filename"() {
    ...
    expect:
    response.statusCode == 200
    response.body.text.trim() == TestConstants.BASE_DIR.resolve("../assets/html/default.htm").text
  }
}
